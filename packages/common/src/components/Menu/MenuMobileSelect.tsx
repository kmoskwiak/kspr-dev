import { styles } from './MenuMobileSelect.css';
...
export const MenuMobileSelect = ({
    ...
    const calculateTextPosition = (index: number) => {
        const rotation = calculatePosition(index);
    ...
    return (
        <div {...props} css={styles} onMouseMove={onDrag}>
                ...
                <ul className={`menu-ring ${active ? 'active' : ''}`} style={{
                    ...
                    {links.map((link, index) => (
                        <li
                            ...
                            data-to={link.url}
                            style={{
                                transform: `translateY(${calculatePosition(index)}px) rotateX(${calculatePosition(index)}deg)`
                            }}
                        >
                            <span className='text'
                                style={{
            ...
            <div>
                {links.map((link, index) => (
                    <div key={link.url} style={{}} className='drop-area' onDragOver={allowDrop} />
                ))}
            </div>

            <div className='menu-select'>
                <input type='range' min='0' max='2' value={sliderValue} onChange={(e) => setSliderValue(parseInt(e.target.value))} />
            </div>
